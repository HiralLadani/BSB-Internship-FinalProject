...
const UserProfileForm = ({ onSubmit, currentProfile }) => {
  const [name, setName] = useState(currentProfile?.name ?? "");
  const [bio, setBio] = useState(currentProfile?.bio ?? "");
  const [email, setEmail] = useState(currentProfile?.contact_email ?? "");
  const [github, setGithub] = useState(currentProfile?.github_username ?? "");

  useEffect(() => {
    // Update form fields when currentProfile changes (e.g., after initial fetch)
    if (currentProfile) {
      setName(currentProfile.name || "");
      setBio(currentProfile.bio || "");
      setEmail(currentProfile.contact_email || "");
      setGithub(currentProfile.github_username || "");
    }
  }, [currentProfile]);

  const handleSubmit = () => {
    if (!name) {
      toast.error("Name is required for your profile.");
      return;
    }
    onSubmit({
      name,
    ...
    actor.update_profile(profile);
    alert("Saved!");
  };

  return (
    <div className="profile-form panel">
      <h3>{currentProfile ? "Update Your Profile" : "Create Your Profile"}</h3>
      <input type="text" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} />
      <textarea placeholder="Bio" value={bio} onChange={(e) => setBio(e.target.value)} />
      <input type="email" placeholder="Contact Email" value={email} onChange={(e) => setEmail(e.target.value)} />
      <input type="text" placeholder="GitHub Username (Optional)" value={github} onChange={(e) => setGithub(e.target.value)} />
      <button onClick={handleSubmit}>{currentProfile ? "Update Profile" : "Create Profile"}</button>
    </div>
  );
};
...
const ProfessorPanel = ({ onCreateCourse, onProposeCourse, myCourses, currentProfile, onUpdateProfile }) => {
  return (
    <div className="panel professor-panel">
      <h2>Professor Dashboard</h2>
      <UserProfileForm onSubmit={onUpdateProfile} currentProfile={currentProfile} />
...
const StudentPanel = ({ currentProfile, onUpdateProfile, coursesForVoting, approvedCourses, myEnrolledCourses, onVoteCourse, onEnrollCourse }) => {
  return (
...
export default function App() {
  ...
  const refreshSessionAndData = async (currentActor, intendedRole = null) => {
    if (!currentActor) {
    ...
    try {
      ...
      if (determinedRole === "Admin") {
        const [loadedUsers, loadedPrincipals, coursesRes] = await Promise.all([
          ...
          currentActor.list_courses(null).catch(() => []), // Admins can see ALL courses
        ]);
        ...
        setMyEnrolledCourses([]); // Admins don't enroll in courses in this context
          ...
          const coursesRes = await currentActor.list_courses(null).catch(() => []); // Professors see their own drafts + proposed/approved
          setAllCourses(coursesRes.map(c => ({
            ...c,
          ...
          const [coursesRes, enrolledCoursesRes] = await Promise.all([
            currentActor.list_courses(null).catch(() => []), // Students see proposed/approved
  ...
  const handleCreateOrUpdateProfile = async (profileData) => {
    if (!actor) { toast.error("Please log in to manage your profile."); return; }
    try {
      ...
      await actor.create_or_update_my_profile(processedProfileData);
      toast.success("Profile saved successfully!");
      await refreshSessionAndData(actor); // Refresh to show updated profile
    } catch (err) {
    ...
    <div className="app">
      ...
      {role === "Professor" && (
        <ProfessorPanel
          ...
          myCourses={allCourses.filter(c => c.professor_id === principalText)} // Filter professor's own courses
